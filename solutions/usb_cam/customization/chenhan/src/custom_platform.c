...
static void _SensorPinmux()
{
    ...
    PINMUX_CONFIG(PWR_GPIO1, PWR_GPIO_1); // rst2
    _GPIOSetValue(4, 1, 1);

    PINMUX_CONFIG(PWR_GPIO2, PWR_GPIO_2); // rst1
#endif
}
...
static void _IRledPinmux(void)
{
	PINMUX_CONFIG(JTAG_CPU_TMS, PWM_7); // IR
	PINMUX_CONFIG(JTAG_CPU_TCK, PWM_6); // 可见光 PWR_GPIO[21]
}
